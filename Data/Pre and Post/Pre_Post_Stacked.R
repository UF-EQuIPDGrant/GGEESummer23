...
####################################################################################################################
###AI EXPERIENCE AND ENJOYMENT###
####################################################################################################################

Pre_Exp_AI <- read_excel("Data/GGEE_23_Pre_Survey_Likert.xlsx", sheet = 7)
Pre_Enj_AI <- read_excel("Data/GGEE_23_Pre_Survey_Likert.xlsx", sheet = 8)

## STACK PRE - EXPERIENCE



ggplot(Pre_Exp_AI, aes(x=Question, y = n, fill = Level, label = round(p, digits=0),"%"))+
  geom_bar(stat="identity", colour='black', size=.5)+
  geom_text(size = 5, position = position_stack(vjust = 0.5))+
  theme_classic()+
  coord_flip()+
...
ggsave(
  filename = "GGEE_23_Summer_Pre_ExpAI_Stacked.png",
  plot = last_plot(),
  device = "png",
  path = "/Users/kristachisholm/Documents/GitHub/GGEESummer23/Graphs/Pre_Survey Responses/",
  scale = 2,
...
ggplot(Pre_Enj_AI, aes(x=Question, y = n, fill = Level, label = round(p, digits=0),"%"))+
  geom_bar(stat="identity", colour='black', size=.5)+
  geom_text(size = 5, position = position_stack(vjust = 0.5))+
  theme_classic()+
  coord_flip()+
...
ggsave(
  filename = "GGEE_23_Summer_Pre_EnjAI_Stacked.png",
  plot = last_plot(),
  device = "png",
  path = "/Users/kristachisholm/Documents/GitHub/GGEESummer23/Graphs/Pre_Survey Responses/",
  scale = 2,
