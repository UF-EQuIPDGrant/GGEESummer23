...
##Stacked bar chart###
######################SKILL LEVEL###########  
Pre_Post_Skill<-read_excel("Pre_Post_Stacked.xlsx", sheet=4)
...
ggsave(
  filename = "GGEE_23_Summer_Pre_Post_Stacked_Skill_0321.png",
  plot = last_plot(),
  device = "png",
  path = "/Users/oliviarandell/Documents/GitHub/GGEESummer23/Graphs/Pre Post Survey/",
  scale = 2,
  width = 6,
  height = 4,
  units = c("in"),
...
####Stacked######
####################Enjoyment########################
Pre_Post_Enjoy<-read_excel("Pre_Post_Stacked.xlsx", sheet=3)
ggplot(Pre_Post_Enjoy)+
  geom_bar(aes(x = Question, y = P, fill = Level),
           ...
           color = "black")+
  #geom_text(aes(label = paste0(round(P, digits=0),"%"),x=Stage, y=P),size = 3, position = position_stack(vjust = 0.5, reverse=TRUE))+
  geom_text(aes(label = round(P, digits=0), x=Question, y = P, group = Level), size = 3, position = position_stack(vjust = 0.5, reverse=TRUE))+
  coord_flip()+
  ...
  theme(legend.title = element_text(size = 14, face = "bold"),
        legend.text = element_text(size = 12))
ggsave(
  filename = "GGEE_23_Summer_Pre_Post_Stacked_Enjoy_0321.png",
  plot = last_plot(),
  device = "png",
  path = "/Users/oliviarandell/Documents/GitHub/GGEESummer23/Graphs/Pre Post Survey/",
  scale = 2,
